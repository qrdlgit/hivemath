...
5. For `validate_result`, manually verify that each step follows, assumptions are sufficient, dependencies support their use, and obvious counterexamples are excluded. Then call `submit_validation`. Use `validated` only when the presented argument establishes the stated result.
